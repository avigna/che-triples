...
def get_maximal_eccentricity(m1, m2, m3, a1, a2, e2, eps_SA1, eps_GR1, eps_Tide1, cos_inc, debug, method):
    # [m1]=[m2]=[m3]=Msun
    # [a1]=[a2]=au
    # [e2]=adim
    # [r1]=[r2]=au
    ...
    e_guess = max(0.99, 1 - np.tanh(eps_GR1))     
        
    # in the next function, what does F means?
    def F(e_m=e_guess,cosi_0=cosi_0, eta=ETA):
        return (1 - e_m**2)*(-3 + eta*cosi_0 + e_m**2*eta**2/4) + 5*(cosi_0 + eta*e_m**2/2)**2 - eps_SA1*(3/8)*( 
            ...
            - 9*(1 - e_m**2)*(cosi_0 + e_m**2*eta/2)) - (eps_GR1*8/3)*((1 - e_m**2)/(e_m**2+1e-8))*(1 - (1 - e_m**2)**(-1/2)) \
            - eps_Tide1*(8/45)*(1 - (1 + 3*e_m**2 + (3/8)*e_m**4)/( (1 - e_m**2)**(9/2) )  )
            
    if method =='brute force':
        if eps_GR1<=2.25:
            e_grid = np.linspace(0.001,0.9999, 10000)
        elif 3.2>eps_GR1> 2.25:
            e_grid = np.logspace(-4,-2, 10000)
        elif eps_GR1>= 3.2:
            e_grid = np.logspace(-7,-3, 10000)
        
        F_vals = []
        for e in e_grid:
            F_vals.append(np.fabs(F(e, cosi_0, ETA)))
        ind = np.argmin(F_vals)
        return e_grid[ind], F_vals[ind]
...
eps_SA1=0.
eps_Tide1=0
eps_GR1=np.logspace(-1.5, 1.5,301)#np.logspace(-2,1,201)
cos_inc=0
debug = False
em=[]
F_vals = []
...
em_guess = [1 - np.arctan(x * n) for x in eps_GR1]
method = 'brute force'
#method = 'fsolve'
for i in range(0,len(eps_GR1)):
    em_to_append, F = get_maximal_eccentricity(m1, m2, m3, a1, a2, e2, eps_SA1, eps_GR1[i], eps_Tide1, cos_inc, debug, method)
    em.append(em_to_append)#%%
    F_vals.append(np.fabs(F))
    
plt.plot(eps_GR1, em, linewidth=3, label='numerical')
plt.plot(eps_GR1, [max(1e-20, x)**0.5 for x in em_analytic2],linewidth=3, label='analytic')
plt.plot(eps_GR1, em_guess,color='black', linewidth=3, label='guess')
plt.plot(eps_GR1,  F_vals, color='red', label='residual')
